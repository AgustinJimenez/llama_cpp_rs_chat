...
Status: IMPLEMENTED 2026-09-11 (proposed by user 2026-09-10)

## What shipped

**Setting:** `max_loaded_models` in the `config` table (migration + `DbSamplerConfig` +
`ChatConfig` + App Settings UI). **Default 2, not 1** — deviating from the proposal below,
deliberately: a cap of 1 disables the per-conversation overflow-worker path (parallel
inference) for everyone, and two 9B models fit a 24 GB card comfortably. It is two *large*
models that starve it, and the VRAM fit check — not the count — is what catches that. Set
1 for strict one-model-at-a-time.

**Enforcement:** `WorkerPool::enforce_slot_cap()` runs *before* `evict_to_fit` in both
spawn paths and in `free_memory_for_load`. It evicts the least-recently-**used** idle
occupant. `WorkerEntry` gained a shared `last_used` timestamp touched by `get()` — using
`created_at` instead would have evicted whichever agent the user activated first, usually
the one they are actively working with. Workers that are *loading* count as occupied, so
two concurrent activations can't both see a free slot.

**Never evicts a generating worker.** If every occupied slot is mid-generation the load is
refused with a message naming the setting, rather than truncating a live response (the
failure class task 001 is about). The eviction loop is bounded rather than `while` — a
failed eviction would otherwise spin forever on a user-triggered load, and a hang is the
worst failure mode in this codebase. On exhausting the rounds it proceeds and lets
`evict_to_fit` guard VRAM.

**UI:** `SlotOccupancyBar` in the agents modal shows `Model slots: N / M` plus a chip per
resident model with its VRAM, so "activating this will unload X" is predictable.

## Verified end-to-end

| Test | Result |
|---|---|
| cap=1, load 9B then 27B | 9B evicted; `slots_used` stayed 1; GPU 11.9 GB → 19.0 GB (27B alone) |
| orphan check after eviction | exactly 3 processes (server + empty default + one worker) |
| generation still works | 27B replied correctly at **30.4 tok/s** |
| cap=2, two models that don't both fit | cap allowed it, **fit check evicted the 27B** — the caveat below, working |
| config round-trip | `POST /api/config` → pool picks up the new cap live, no restart |
| slot bar | rendered `Model slots: 1 / 2` with a `Qwen3.5-9B 8.9G` chip |

The cap=2 case is the important one: a slot was free, so the count permitted the load, and
only the free-VRAM check prevented the over-subscription. Count alone would not have.

## Open questions still unanswered

- **Sub-agents were not investigated.** `spawn_agent`/`sub_agent.rs` was not checked against
  the cap. A sub-agent reusing the parent's model should reuse the worker (the existing
  same-model reuse scan in `spawn_worker_for_agent` probably covers it) but this is
  unverified — if a sub-agent spawns a distinct model it will now consume a slot.
- Eager vs lazy unload on switch: unchanged, still eager via eviction.
