//! `clipboard_image` tool — read or write an image from/to the clipboard.
...
pub fn tool_clipboard_image(args: &Value) -> NativeToolResult {
    ...
    match action {
        ...
        other => crate::tool_error("clipboard_image", format!("unknown action '{other}', use 'read' or 'write'")),
    }
}
...
fn clipboard_image_read() -> NativeToolResult {
    unsafe {
        if win32::OpenClipboard(0) == 0 {
            return crate::tool_error("clipboard_image", "could not open clipboard");
        }

        let handle = win32::GetClipboardData(win32::CF_DIB);
        if handle == 0 {
            win32::CloseClipboard();
            return crate::tool_error("clipboard_image", "no image in clipboard (CF_DIB format)");
        }

        let size = win32::GlobalSize(handle);
        if size == 0 {
            win32::CloseClipboard();
            return crate::tool_error("clipboard_image", "GlobalSize returned 0");
        }

        let ptr = win32::GlobalLock(handle);
        if ptr.is_null() {
            win32::CloseClipboard();
            return crate::tool_error("clipboard_image", "GlobalLock failed");
        }
        ...
        if data.len() < 40 {
            ...
            win32::CloseClipboard();
            return crate::tool_error("clipboard_image", "DIB data too small");
        }
        ...
        if compression != 0 {
            ...
            win32::GlobalUnlock(handle);
            win32::CloseClipboard();
            return crate::tool_error("clipboard_image", format!("unsupported DIB compression type {compression} (only BI_RGB supported)"));
        }
        ...
        if pixel_offset + row_size * height as usize > data.len() {
            win32::GlobalUnlock(handle);
            win32::CloseClipboard();
            return crate::tool_error("clipboard_image", "DIB pixel data exceeds buffer size");
        }
        ...
        if let Err(e) = image::ImageEncoder::write_image(
            ...
            return crate::tool_error("clipboard_image", format!("encoding PNG: {e}"));
        }
...
fn clipboard_image_write(monitor_idx: usize) -> NativeToolResult {
    // Capture the screen
    let monitors = match crate::validated_monitors("clipboard_image", monitor_idx) {
        Ok(m) => m,
        Err(e) => return e,
    };
    let img = match monitors[monitor_idx].capture_image() {
        Ok(i) => i,
        Err(e) => return crate::tool_error("clipboard_image", format!("capturing screen: {e}")),
    };
    ...
    unsafe {
        ...
        if hmem == 0 {
            return crate::tool_error("clipboard_image", "GlobalAlloc failed");
        }
        ...
        if total_size == 0 {
            ...
            return crate::tool_error("clipboard_image", "zero-size allocation");
        }
        ...
        if result == 0 {
            crate::tool_error("clipboard_image", "SetClipboardData failed")
        } else {
...
fn clipboard_image_read() -> NativeToolResult {
    let mut clipboard = match arboard::Clipboard::new() {
        Ok(cb) => cb,
        Err(e) => return crate::tool_error("clipboard_image", format!("could not open clipboard: {e}")),
    };

    let img_data = match clipboard.get_image() {
        Ok(data) => data,
        Err(e) => return crate::tool_error("clipboard_image", format!("no image in clipboard: {e}")),
    };
    ...
    let rgba_img = match image::RgbaImage::from_raw(width, height, img_data.bytes.into_owned()) {
        Some(img) => img,
        None => return crate::tool_error("clipboard_image", "failed to create image from clipboard data"),
    };
...
fn clipboard_image_write(monitor_idx: usize) -> NativeToolResult {
    ...
    match clipboard.set_image(img_data) {
        ...
        Err(e) => crate::tool_error("clipboard_image", format!("failed to set clipboard image: {e}")),
    }
}

#[cfg(not(any(windows, target_os = "macos", target_os = "linux")))]
pub fn tool_clipboard_image(_args: &Value) -> NativeToolResult {
    crate::tool_error("clipboard_image", "not available on this platform")
}
