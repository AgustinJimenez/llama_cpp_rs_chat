...
pub const DEFAULT_THREAD_TIMEOUT: Duration = Duration::from_secs(20);
...
pub const SCREENSHOT_MAX_DIM: u32 = 1280;
...
#[allow(dead_code)]
pub fn tool_error(tool: &str, msg: impl std::fmt::Display) -> NativeToolResult {
    NativeToolResult::text_only(format!("Error [{tool}]: {msg}"))
}

/// Format a platform-not-supported error.
#[allow(dead_code)]
pub fn tool_not_supported(tool: &str) -> NativeToolResult {
...
pub fn cached_monitors() -> Result<Vec<xcap::Monitor>, String> {
...
pub fn encode_image_to_png(img: &image::RgbaImage) -> Result<Vec<u8>, String> {
...
pub fn resize_screenshot_for_vision(png_bytes: &[u8], max_dim: u32) -> Vec<u8> {
...
#[allow(dead_code)]
pub fn encode_as_jpeg(png_bytes: &[u8], quality: u8) -> Vec<u8> {
    let img = match image::load_from_memory(png_bytes) {
        Ok(i) => i,
        Err(_) => return png_bytes.to_vec(),
    };
...
pub fn optimize_screenshot_for_vision(png_bytes: &[u8]) -> Vec<u8> {
...
pub fn parse_int(v: &Value) -> Option<i64> {
    v.as_i64()
...
pub fn parse_bool(v: &Value, default: bool) -> bool {
    v.as_bool()
...
pub fn with_enigo<F, T>(f: F) -> Result<T, String>
where
...
pub fn snap_coordinates(x: i32, y: i32) -> (i32, i32) {
...
pub fn apply_dpi_scaling(x: i32, y: i32, dpi_aware: bool) -> (i32, i32) {
    if dpi_aware {
        #[cfg(windows)]
...
#[cfg(windows)]
pub fn check_modal_dialog() -> Option<String> {
...
pub fn parse_key_combo(key_str: &str) -> Result<(Vec<Key>, Key), String> {
...
pub fn update_screenshot_cache(raw: Vec<u8>, png: Vec<u8>) {
    LAST_SCREENSHOT.with(|cell| {
...
#[allow(dead_code)]
pub fn wait_for_screen_settle(timeout_ms: u64, poll_ms: u64) -> bool {
