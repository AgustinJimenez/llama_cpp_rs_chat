            ...
            conn.query_row(
                ...
                |row| {
                    ...
                    Ok(DbSamplerConfig {
                        ...
                        web_browser_backend: row.get(33)?,
                        models_directory: row.get(34)?,
                        seed: row.get::<_, Option<i32>>(35)?.unwrap_or(-1),
                        n_ubatch: row.get::<_, Option<u32>>(36)?.unwrap_or(512),
                        n_threads: row.get::<_, Option<i32>>(37)?.unwrap_or(0),
                        n_threads_batch: row.get::<_, Option<i32>>(38)?.unwrap_or(0),
                        rope_freq_base: row.get::<_, Option<f64>>(39)?.unwrap_or(0.0) as f32,
                        rope_freq_scale: row.get::<_, Option<f64>>(40)?.unwrap_or(0.0) as f32,
                        use_mlock: row.get::<_, Option<i32>>(41)?.unwrap_or(0) != 0,
                        use_mmap: row.get::<_, Option<i32>>(42)?.unwrap_or(1) != 0,
                        main_gpu: row.get::<_, Option<i32>>(43)?.unwrap_or(0),
                        split_mode: row.get::<_, Option<String>>(44)?.unwrap_or_else(|| "layer".to_string()),
                        ...
                        safe_tool_injection: row.get::<_, Option<i32>>(49)?.unwrap_or(0) != 0,
                        telegram_bot_token: row.get(50)?,
                        telegram_chat_id: row.get(51)?,
                        provider_api_keys: row.get(52)?,
                        max_tool_calls: row.get::<_, Option<i32>>(53)?.unwrap_or(2000),
                        loop_detection_limit: row.get::<_, Option<i32>>(54)?.unwrap_or(15),
                    })
                },
            )
    ...
    pub fn update_config(&self, config: &DbSamplerConfig) -> Result<(), String> {
        ...
        conn.execute(
            "UPDATE config SET
             ...
             web_browser_backend = ?34,
             models_directory = ?35,
             seed = ?36, n_ubatch = ?37, n_threads = ?38, n_threads_batch = ?39,
             rope_freq_base = ?40, rope_freq_scale = ?41,
             use_mlock = ?42, use_mmap = ?43, main_gpu = ?44, split_mode = ?45,
             use_rtk = ?46,
             use_htmd = ?47,
             tag_pairs = ?48,
             proactive_compaction = ?49,
             safe_tool_injection = ?50,
             telegram_bot_token = ?51,
             telegram_chat_id = ?52,
             provider_api_keys = ?53,
             max_tool_calls = ?54,
             loop_detection_limit = ?55,
             updated_at = ?56
             WHERE id = 1",
            params![
                config.sampler_type,
