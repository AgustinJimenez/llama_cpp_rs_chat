...
use llama_chat_types::ipc_types::McpServerStatus;
...
// Implement the McpManagerOps trait from llama-chat-tools crate.
// This lets the tools crate call MCP operations without depending on the
// concrete McpManager type (avoids circular dependency).
impl llama_chat_tools::McpManagerOps for McpManager {
    ...
    fn call_tool(&self, qualified_name: &str, args: Value) -> Result<String, String> {
        self.call_tool(qualified_name, args)
    }

    fn get_server_statuses(&self) -> Vec<McpServerStatus> {
        self.get_server_statuses()
    }

    fn get_tool_definitions(&self) -> Vec<llama_chat_tools::McpToolDefInfo> {
        self.get_tool_definitions()
            .into_iter()
            .map(|td| llama_chat_tools::McpToolDefInfo {
