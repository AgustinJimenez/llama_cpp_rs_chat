...
fn main() {
    ...
    // Use TURBO2_0 (K) and TURBO3_0 (V) — same as the app config
    let kv_type_k = llama_cpp_2::context::params::KvCacheType::Unknown(43); // TURBO2_0
    let kv_type_v = llama_cpp_2::context::params::KvCacheType::Unknown(41); // TURBO3_0
    println!("KV cache types: K=TURBO2_0(43), V=TURBO3_0(41)");
    let ctx_params = LlamaContextParams::default()
        .with_n_ctx(Some(n_ctx))
        .with_flash_attention_policy(1)
        .with_offload_kqv(true)
        .with_type_k(kv_type_k)
        .with_type_v(kv_type_v);
    let mut ctx = model
