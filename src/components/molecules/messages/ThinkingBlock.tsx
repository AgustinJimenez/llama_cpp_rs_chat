  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: '1px solid hsl(220 8% 28%)' }}>
      ...
      >
        ...
        <span className="text-xs font-medium text-foreground flex-1">{thinkingLabel}</span>
        {!!isOpen && <ChevronDown className="h-3.5 w-3.5 text-foreground flex-shrink-0" />}
        {!isOpen && <ChevronRight className="h-3.5 w-3.5 text-foreground flex-shrink-0" />}
      </button>
      {!!isOpen && (
        <pre
