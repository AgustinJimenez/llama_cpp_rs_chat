...
export const ModelMetadataDisplay: React.FC<ModelMetadataDisplayProps> = ({ modelInfo }) => (
  <ParamGroup title="Model Metadata" collapsible defaultExpanded={false} freeLayout className="mt-3">
    <div className="space-y-3 text-xs max-h-96 overflow-y-auto">
      ...
      <div className="space-y-1">
        ...
        {modelInfo.general_name ? <p><strong>Model Name:</strong> <span className="text-muted-foreground">{modelInfo.general_name}</span></p> : null}
      ...
      {/* Model Details */}
      {(modelInfo.description || modelInfo.author || modelInfo.organization || modelInfo.version || modelInfo.license) ? <div className="space-y-1 pt-2 border-t">
          <h4 className="font-semibold text-sm mb-2">Model Details</h4>
          {modelInfo.description ? <p><strong>Description:</strong> <span className="text-muted-foreground">{modelInfo.description}</span></p> : null}
          {modelInfo.author ? <p><strong>Author:</strong> <span className="text-muted-foreground">{modelInfo.author}</span></p> : null}
          {modelInfo.organization ? <p><strong>Organization:</strong> <span className="text-muted-foreground">{modelInfo.organization}</span></p> : null}
          {modelInfo.version ? <p><strong>Version:</strong> <span className="text-muted-foreground">{modelInfo.version}</span></p> : null}
          {modelInfo.license ? <p><strong>License:</strong> <span className="text-muted-foreground">{modelInfo.license}</span></p> : null}
          {modelInfo.url ? <p><strong>URL:</strong> <a href={modelInfo.url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline break-all">{modelInfo.url}</a></p> : null}
          {modelInfo.repo_url ? <p><strong>Repository:</strong> <a href={modelInfo.repo_url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline break-all">{modelInfo.repo_url}</a></p> : null}
        </div> : null}

      {/* Architecture Specs */}
      {(modelInfo.context_length || modelInfo.block_count || modelInfo.embedding_length || modelInfo.feed_forward_length) ? <div className="space-y-1 pt-2 border-t">
          ...
          {modelInfo.block_count ? <p><strong>Block Count (Layers):</strong> <span className="text-muted-foreground">{modelInfo.block_count}</span></p> : null}
          {modelInfo.embedding_length ? <p><strong>Embedding Length:</strong> <span className="text-muted-foreground">{modelInfo.embedding_length}</span></p> : null}
          {modelInfo.feed_forward_length ? <p><strong>FFN Length:</strong> <span className="text-muted-foreground">{modelInfo.feed_forward_length}</span></p> : null}
          {modelInfo.attention_head_count ? <p><strong>Attention Heads:</strong> <span className="text-muted-foreground">{modelInfo.attention_head_count}</span></p> : null}
          {modelInfo.attention_head_count_kv ? <p><strong>KV Heads:</strong> <span className="text-muted-foreground">{modelInfo.attention_head_count_kv}</span></p> : null}
      ...
      {/* Tokenizer Info */}
      {(modelInfo.tokenizer_model || modelInfo.bos_token_id || modelInfo.eos_token_id || modelInfo.chat_template) ? <div className="space-y-1 pt-2 border-t">
          <h4 className="font-semibold text-sm mb-2">Tokenizer Information</h4>
          {modelInfo.tokenizer_model ? <p><strong>Tokenizer Type:</strong> <span className="text-muted-foreground">{modelInfo.tokenizer_model}</span></p> : null}
          {modelInfo.bos_token_id ? <p><strong>BOS Token ID:</strong> <span className="text-muted-foreground font-mono">{modelInfo.bos_token_id}</span></p> : null}
          {modelInfo.eos_token_id ? <p><strong>EOS Token ID:</strong> <span className="text-muted-foreground font-mono">{modelInfo.eos_token_id}</span></p> : null}
          {modelInfo.padding_token_id ? <p><strong>Padding Token ID:</strong> <span className="text-muted-foreground font-mono">{modelInfo.padding_token_id}</span></p> : null}
          {modelInfo.chat_template ? <div>
      ...
      {/* All GGUF Metadata */}
      {modelInfo.gguf_metadata && Object.keys(modelInfo.gguf_metadata).length > 0 ? <div className="space-y-1 pt-2 border-t">
