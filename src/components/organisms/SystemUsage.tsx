...
const MiniGraph = ({ data, color }: MiniGraphProps) => {
...
const LargeGraph = ({ data, color, gbText }: LargeGraphProps) => {
  if (data.length === 0) return null;
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-lg border border-border bg-muted p-2">
            <LargeGraph
              data={gpuHistory}
              color="#22c55e"
              gbText={`${vramUsedGb.toFixed(1)} / ${effectiveVramGb.toFixed(1)} GB`}
            />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-lg border border-border bg-muted p-2">
            <LargeGraph
              data={ramHistory}
              color="#a855f7"
              gbText={`${ramUsedGb.toFixed(1)} / ${effectiveRamGb.toFixed(1)} GB`}
            />
          </div>
        </div>
    ...
    <div className="flex items-center gap-3 rounded-xl border border-border bg-muted px-3 py-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="rounded-lg border border-border bg-background px-2 py-1">
          <MiniGraph data={cpuHistory} color="#3b82f6" />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="rounded-lg border border-border bg-background px-2 py-1">
          <MiniGraph data={gpuHistory} color="#22c55e" />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="rounded-lg border border-border bg-background px-2 py-1">
          <MiniGraph data={ramHistory} color="#a855f7" />
        </div>
      </div>
    </div>
  );
