            ...
            conn.query_row(
                ...
                |row| {
                    ...
                    Ok(DbSamplerConfig {
                        ...
                        seed: row.get::<_, Option<i32>>(34)?.unwrap_or(-1),
                        n_ubatch: row.get::<_, Option<u32>>(35)?.unwrap_or(512),
                        n_threads: row.get::<_, Option<i32>>(36)?.unwrap_or(0),
                        n_threads_batch: row.get::<_, Option<i32>>(37)?.unwrap_or(0),
                        rope_freq_base: row.get::<_, Option<f64>>(38)?.unwrap_or(0.0) as f32,
                        rope_freq_scale: row.get::<_, Option<f64>>(39)?.unwrap_or(0.0) as f32,
                        use_mlock: row.get::<_, Option<i32>>(40)?.unwrap_or(0) != 0,
                        use_mmap: row.get::<_, Option<i32>>(41)?.unwrap_or(1) != 0,
                        main_gpu: row.get::<_, Option<i32>>(42)?.unwrap_or(0),
                        split_mode: row.get::<_, Option<String>>(43)?.unwrap_or_else(|| "layer".to_string()),
                        use_rtk: row.get::<_, Option<i32>>(44)?.unwrap_or(0) != 0,
                        use_htmd: row.get::<_, Option<i32>>(45)?.unwrap_or(0) != 0,
                        tag_pairs: row.get(46)?,
                    })
                },
            )
    ...
    pub fn update_config(&self, config: &DbSamplerConfig) -> Result<(), String> {
        ...
        conn.execute(
            "UPDATE config SET
             ...
             seed = ?35, n_ubatch = ?36, n_threads = ?37, n_threads_batch = ?38,
             rope_freq_base = ?39, rope_freq_scale = ?40,
             use_mlock = ?41, use_mmap = ?42, main_gpu = ?43, split_mode = ?44,
             use_rtk = ?45,
             use_htmd = ?46,
             tag_pairs = ?47,
             updated_at = ?48
