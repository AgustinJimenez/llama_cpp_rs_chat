...
pub fn tool_find_and_click_text(args: &Value) -> NativeToolResult {
    ...
    // Capture screen
    let monitors = match super::validated_monitors("find_and_click_text", monitor_idx) {
        Ok(m) => m,
        Err(e) => return e,
    };
...
pub fn tool_wait_for_text_on_screen(args: &Value) -> NativeToolResult {
    ...
    loop {
        ...
        let monitors = match super::validated_monitors("wait_for_text_on_screen", monitor_idx) {
            Ok(m) => m,
            Err(e) => return e,
        };
        let img = match monitors[monitor_idx].capture_image() {
...
/// Combines wait_for_screen_change + wait_for_text_on_screen into one tool.
#[cfg(any(windows, target_os = "macos", target_os = "linux"))]
pub fn tool_smart_wait(args: &Value) -> NativeToolResult {
    ...
    if text.is_none() && mode == "all" {
        return super::tool_error("smart_wait", "'text' is required when mode is 'all'");
    }

    // Capture baseline
    let monitors = match super::validated_monitors("smart_wait", monitor_idx) {
        Ok(m) => m,
        Err(e) => return e,
    };
    let baseline_img = match monitors[monitor_idx].capture_image() {
        Ok(i) => i,
        Err(e) => return super::tool_error("smart_wait", format!("capture: {e}")),
    };
    ...
    // If no text to search for, mark text_found as true immediately
    if text.is_none() {
        text_found = true;
    }
    ...
    loop {
        if let Err(e) = super::ensure_desktop_not_cancelled() {
            return super::tool_error("smart_wait", e);
        }
        ...
        // Re-enumerate monitors each iteration to handle monitor changes
        ...
        if !screen_changed {
            if let Ok(img) = monitors[monitor_idx].capture_image() {
                let diff = super::pixel_diff_pct(&baseline_raw, img.as_raw());
        ...
        // Check text via OCR (using the cross-platform ocr_find_text dispatcher)
        if !text_found {
            if let Some(ref search_text) = text {
                if let Ok(img) = monitors[monitor_idx].capture_image() {
                    let search = search_text.to_lowercase();
                    let ocr_result = super::spawn_with_timeout(
                        std::time::Duration::from_secs(10),
                        move || {
                            super::ocr_tools::ocr_find_text(&img, &search, 0.0, 0.0)
                        },
                    ).and_then(|r| r);
        ...
        let done = match mode {
            ...
            _ => screen_changed || text_found, // "any"
        };
        if done {
            break;
        }

        let sleep_ms = super::adaptive_poll_ms(attempt, poll_ms, poll_ms * 4);
        if let Err(e) = super::interruptible_sleep(std::time::Duration::from_millis(sleep_ms)) {
            return super::tool_error("smart_wait", e);
        }
        attempt += 1;
    }
...
pub fn tool_smart_wait(_args: &Value) -> NativeToolResult {
    super::tool_error("smart_wait", "not available on this platform")
}
...
pub fn tool_click_and_verify(args: &Value) -> NativeToolResult {
    let click_text = match args.get("click_text").and_then(|v| v.as_str()) {
        Some(t) => t,
        None => return super::tool_error("click_and_verify", "'click_text' is required"),
    };
    let expect_text = match args.get("expect_text").and_then(|v| v.as_str()) {
        Some(t) => t.to_string(),
        None => return super::tool_error("click_and_verify", "'expect_text' is required"),
    };
    let timeout_ms = args.get("timeout_ms").and_then(parse_int).unwrap_or(5000).clamp(500, 30000) as u64;
    let monitor_idx = args.get("monitor").and_then(parse_int).unwrap_or(0) as usize;

    // Step 1: Find and click the target text (reuse find_and_click_text)
    ...
    // Step 2: Wait for expected text to appear via smart_wait
...
pub fn tool_click_and_verify(_args: &Value) -> NativeToolResult {
    super::tool_error("click_and_verify", "not available on this platform")
}
...
mod tests {
    ...
    // ─── smart_wait parameter validation ─────────────────────────────────
    ...
    fn test_smart_wait_mode_any_without_text_starts() {
        // mode "any" without text should start (screen-change-only mode)
        // It will timeout quickly but shouldn't error on missing text
        let args = serde_json::json!({
            "mode": "any",
            "timeout_ms": 500
        });
        let result = tool_smart_wait(&args);
        // Should NOT be a parameter error — either completes or times out
        assert!(!result.text.contains("'text' is required"));
    }

    #[test]
    fn test_smart_wait_timeout_clamped() {
        // timeout_ms < 500 should be clamped to 500
        let args = serde_json::json!({
            "timeout_ms": 100,
            "text": "x"
        });
        let result = tool_smart_wait(&args);
        // Should still run (clamped to 500ms) — won't find text but shouldn't crash
        assert!(!result.text.contains("Error [smart_wait]: 'text'"));
    }

    // ─── click_and_verify parameter validation ───────────────────────────
