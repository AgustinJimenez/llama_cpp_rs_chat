...
/// WebSocket handler for Camofox tab screencast.
/// Streams JPEG frames at ~10 FPS as binary frames.
/// Skips identical frames (by byte length) to avoid redundant updates.
pub async fn handle_camofox_screencast_ws(
    upgraded: Upgraded,
    tab_id: String,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let ws_stream = WebSocketStream::from_raw_socket(
    ...
    sys_info!("[WS_SCREENCAST] Started for tab {}", tab_id);

    let mut frame_interval = tokio::time::interval(Duration::from_millis(100)); // 10 FPS
        ...
        tokio::select! {
            _ = frame_interval.tick() => {
                ...
                let frame = tokio::task::spawn_blocking(move || {
                    crate::web::browser::camofox::take_tab_screenshot_jpeg(&tab_id_clone, 65)
                }).await.ok().flatten();
                if let Some(bytes) = frame {
                    consecutive_failures = 0;
                    // Skip identical frames (very rough dedup by length)
                    ...
                    // Tab is gone or Camofox is down — give up after 3 failures
                    consecutive_failures += 1;
                    if consecutive_failures >= 3 {
                        sys_info!("[WS_SCREENCAST] Tab {} gone, closing stream", tab_id);
                        let _ = ws_sender.send(WsMessage::Text(
                            "{\"type\":\"tab_closed\"}".to_string()
                        )).await;
            ...
            ws_msg = ws_receiver.next() => {
                match ws_msg {
                    ...
                    Some(Ok(WsMessage::Ping(d))) => { let _ = ws_sender.send(WsMessage::Pong(d)).await; }
                    Some(Err(_)) => break,
                    _ => {}
                }
            }
    ...
    sys_info!("[WS_SCREENCAST] Stopped for tab {}", tab_id);
    Ok(())
}
