...
TTS Notifications (KittenTTS): The user has a KittenTTS installation at /Users/agusj/repo/KittenTTS/ with Claude Code hooks that speak events aloud. Setup: (1) Ensure the venv exists: "cd /Users/agusj/repo/KittenTTS && python3 -m venv .venv && .venv/bin/pip install -e . soundfile". (2) The hook script is /Users/agusj/repo/KittenTTS/claude_hook.sh — it reads JSON from stdin, switches on `hook_event_name` to pick a message, and speaks it via play.py. (3) Registered in ~/.claude/settings.json for these hooks: Notification (dynamic message), Stop ("I am done!"), PostToolUseFailure ("{tool} failed!"), SubagentStop ("Sub-task complete."), TaskCompleted ("Task done: {subject}"). PreToolUse/PostToolUse are intentionally omitted (too noisy). To test: echo '{"hook_event_name":"Stop"}' | /Users/agusj/repo/KittenTTS/claude_hook.sh. Voices: Bella, Jasper, Luna, Bruno, Rosie, Hugo, Kiki, Leo (default: Kiki, speed: 1.2).
